function [Z,E,F,Z_hat,converge_Z,converge_Z_G,history] = solve_problem(X,nC,alpha,lambda,yita)
...
%% ============================== Upadate Z^k =============================
      ...
      for k =1:nV
          [dk,~]= size(X{k});
          tmp = X{k}'*Y{k} + mu*X{k}'*X{k} +  rho*J{k}- mu*X{k}'*E{k} - W{k} -(alpha/ww(k))*tem_P ;
%           tmp2 = pinv(rho*eye(N,N))- pinv(rho*eye(N,N))*mu*X{k}'* pinv(eye(dk,dk)+X{k}*pinv(rho*eye(N,N))*mu*X{k}')*X{k}*pinv(rho*eye(N,N));
%            Z{k}=tmp2*tmp;
          Z{k}=inv(rho*eye(N,N)+ mu*X{k}'*X{k})*tmp;
          temZ = temZ + (1/ww(k))*(abs(Z{k})+(abs(Z{k}))')./2;
          Z_view{k} = (abs(Z{k})+(abs(Z{k}))')./2;
...
%         J_tensor = reshape(j, sX);
                ...
                [J_tensor,objV] = solve_G(Z_tensor + 1/rho*W_tensor,rho,sX,yita);
                j = J_tensor(:);
%% ============================== Upadate W ===============================
        w = w + rho*(z - j);
...
%% ============================== Upadate F ===============================
     ...
     gg=[];
     for i = 1:nV
      DN_v = diag( 1./sqrt(sum(Z_view{i})+eps) );
      LapN_v = speye(N) - DN * Z_view{i} * DN;
      gg(i)=sqrt(trace(F'*LapN_v*F));
     end
     ww = gg/sum(gg);

%% ============================== Recording ===============================
   objV = objV + 2*alpha*trace(F'*LapN*F);
